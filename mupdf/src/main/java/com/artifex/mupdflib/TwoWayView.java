...
public class TwoWayView extends AdapterView<ListAdapter> implements
    ...
    private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
        ...
        if (selectedView != null && selectedView.hasFocus()) {
            ...
            if (direction == View.FOCUS_DOWN || direction == View.FOCUS_RIGHT) {
                ...
                final int end = getEndEdge() - (fadingEdgeShowing ? getArrowScrollPreviewLength()
                        : 0);
    ...
    private boolean arrowScrollImpl(int direction) {
        ...
        final ArrowScrollFocusResult focusResult = (mItemsCanFocus ? arrowScrollFocused
                (direction) : null);
        if (focusResult != null) {
     ...
     *
     * @param direction          either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN} or
     *                           {@link View#FOCUS_LEFT} or {@link View#FOCUS_RIGHT} depending on
     *                           the
     *                           current view orientation.
                            ...
                            if (newPos >= 0) {
                                ...
                                if (selectablePos == newPos) {
                                    ...
                                    if (mSyncSize == getSize()) {
                                        ...
                                        // We are not the same height as when the selection was
                                        // saved, so
     ...
     *                                     to have
     *                                     at least 3 completely visible children, and in
     *                                     portrait this
     *                                     will most likely fit; but in landscape there could be
     *                                     times
     *                                     when even 2 children can not be completely shown, so a
     *                                     value
    ...
    private View obtainView(int position, boolean[] isScrap) {
        ...
        if (ViewCompat.getImportantForAccessibility(child) == ViewCompat
